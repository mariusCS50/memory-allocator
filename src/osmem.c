...
// Initializes the metadata for a block of memory
...
// Checks if a system call failed to execute
...
// Preallocates the heap with a bigger chunk to reduce the number of syscalls
void preallocate_heap(void)
{
	void *heap = sbrk(MMAP_THRESHOLD);

	if (syscall_fail(heap, "heap preallocation failed"))
		return;
...
// Coalesces the current block with its following free blocks
void coalesce_current_block(struct block_meta *block, size_t new_payload_size)
{
	while (block && block->next && block->next->status == STATUS_FREE && block->size < new_payload_size) {
		...
		set_block_metadata(block, block->size + METADATA_SIZE + block->next->size, STATUS_FREE,
											 block->prev, block->next->next);
	}
}
...
void coalesce_all_free_blocks(struct block_meta *block)
{
	while (block && block->next) {
		if (block->status == STATUS_FREE && block->next->status == STATUS_FREE) {
			last_block = (block->next == last_block ? block : last_block);
			set_block_metadata(block, block->size + block->next->size + METADATA_SIZE, STATUS_FREE,
												 block->prev, block->next->next);
		} else {
			block = block->next;
		}
...
// Finds the best available block that fits the requested payload size
void *find_best_available_space(struct block_meta *block, size_t payload_size)
{
	...
	for (; block != NULL; block = block->next) {
		if ((block->status == STATUS_FREE && block->size >= payload_size) &&
...
// Appends a new block of memory to the end of the heap
void *append_new_block(size_t payload_size)
{
	void *ptr = sbrk(payload_size + METADATA_SIZE);

	if (syscall_fail(ptr, "new block space allocation failed"))
		return NULL;
...
// Extends the size of the last allocated block to fit the new payload size
void *extend_last_block(size_t payload_size)
{
	void *ptr = sbrk(payload_size - last_block->size);

	if (syscall_fail(ptr, "extend last free block failed"))
		return NULL;
...
// Checks if the block can be split and splits it if possible
void check_block_for_splitting(struct block_meta *block, size_t payload_size)
{
	if (block->size - payload_size > METADATA_SIZE) {
		struct block_meta *new_block = (struct block_meta *)((char *)block + METADATA_SIZE + payload_size);
...
void *os_malloc(size_t size)
{
	...
	if (block_size <= MMAP_THRESHOLD) {
		...
		if (syscall_fail(new_block, "new block space allocation failed"))
			return NULL;
...
void os_free(void *ptr)
{
	...
	if (block->status == STATUS_MAPPED) {
		long call = munmap(block, block->size + METADATA_SIZE);

		if (syscall_fail((void *)call, "free memory block failed"))
			return;
	}
}
